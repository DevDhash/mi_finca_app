...
export class StorageAdapter implements PhotoStorage {
  ...
  constructor(
    http: HttpTransport,
    claimedJob: CleanupJob,
    deadline: () => number,
  ) {
    const namespace = deriveNamespace(claimedJob);
    if (!namespace || !validJobMetadata(claimedJob)) {
      throw new Error("INVALID_CLAIM");
    }
    this.#job = Object.freeze({ ...claimedJob });
    this.#folder = namespace.folder;
    this.#http = http;
  ...
  async list(
    ...
    if (
      folder !== this.#folder || options?.limit !== 100 ||
      options?.offset !== 0 ||
      options?.sortBy?.column !== "name" || options?.sortBy?.order !== "asc" ||
      "search" in options
    ) return malformed();
    const r = await this.#http.request(
      `/storage/v1/object/list/${PHOTO_BUCKET}`,
      "POST",
      {
        prefix: folder,
        limit: 100,
    ...
    return {
      ok: true,
      value: r.value.map((entry): StorageEntry => {
        if (!record(entry) || typeof entry.name !== "string") {
          return { kind: "unexpected", name: "" };
        }
        const kind = typeof entry.id === "string" && entry.id.length > 0 &&
  ...
  async remove(paths: readonly string[]): Promise<AdapterResult<unknown>> {
    if (
      !Array.isArray(paths) || paths.length < 1 || paths.length > 100 ||
      paths.some((p) =>
        typeof p !== "string" || !p.startsWith(this.#folder + "/")
      )
    ) return malformed();
    const validated = pagePaths(
      this.#job,
      paths.map((p) => ({
        kind: "file",
        name: p.slice(this.#folder.length + 1),
      })),
    );
    if (!validated || validated.some((p, i) => p !== paths[i])) {
      return malformed();
    }
    const r = await this.#http.request(
