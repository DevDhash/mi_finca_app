...
export function validSecret(value: unknown): value is string {
  return typeof value === "string" &&
    /^sb_secret_[A-Za-z0-9_-]{20,256}(?![\s\S])/.test(value);
}
...
export function readConfig(env: Environment): WorkerConfig | null {
  try {
    ...
    const number = (
      ...
      const text = env(name);
      if (text !== undefined && !/^[1-9]\d{0,5}(?![\s\S])/.test(text)) {
    ...
    const config = {
      ...
      discoveryLimit: number(
        "ANIMAL_PHOTO_CLEANUP_DISCOVERY_LIMIT",
        1000,
        1,
        5000,
      ),
      invocationMs: number(
        "ANIMAL_PHOTO_CLEANUP_INVOCATION_MS",
        90000,
        15000,
        90000,
      ),
    ...
    if (
      config.invocationMs <=
        config.requestMs * 2 + config.finishMs + config.skewMs
    ) return null;
