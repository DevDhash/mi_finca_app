 ...
 */
export async function authenticate(request: Request, expected: string): Promise<boolean> {
  const supplied = request.headers.get("apikey");
  if (!validSecret(expected) || !validSecret(supplied)) return false;
  const encoder = new TextEncoder();
  const algorithm = { name: "HMAC", hash: "SHA-256" };
  const expectedKey = await crypto.subtle.importKey("raw", encoder.encode(expected), algorithm, false, ["sign"]);
