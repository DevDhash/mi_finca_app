...
test("deadline normal response", async () => {
  const r = await withDeadline(() => Promise.resolve(42), Date.now() + 1000);
  ok(r.ok);
  equal(r.value, 42);
});
...
for (const cooperative of [true, false]) {
  test(`never responding request aborts; cooperative=${cooperative}`, async () => {
    let signal: AbortSignal | undefined;
    const r = await withDeadline((s) => {
      signal = s;
    ...
    ok(!r.ok);
    equal(r.error.kind, "timeout");
    ok(signal?.aborted);
  });
}
for (const stage of ["list", "remove"]) {
  test(`abort during ${stage}`, async () => {
    let signal: AbortSignal | undefined;
    ...
    const r = stage === "list"
      ...
      : await storage.remove([
        `${folder}/aaaaaaaa-0000-4000-8000-000000000001.jpg`,
      ]);
    ok(!r.ok);
    equal(r.error.kind, "timeout");
    ok(signal?.aborted);
...
for (
  const [label, leaseMs, invocationMs, allowed] of [
    ["expired", -1, 60000, false],
    ["near expiry", 3000, 60000, false],
    ["invocation first", 60000, 3000, false],
    ["lease first", 3000, 60000, false],
    ["sufficient", 60000, 90000, true],
  ] as const
) {
  test(`budget ${label}`, () => {
    const b = leaseBudget(
      new Date(base + leaseMs).toISOString(),
      base + invocationMs,
      () => base,
    );
    equal(b.budget.canStartList(), allowed);
    equal(b.budget.canStartRemove(), allowed);
...
test("budget gates change as clock advances; preserves finish reserve", () => {
  let now = base;
  const b = leaseBudget(
    new Date(base + 60000).toISOString(),
    base + 90000,
    () => now,
  );
  equal(b.storageDeadline(), base + 5000);
  now += 50000;
  equal(b.budget.canStartList(), false);
  equal(b.budget.canFinish(), true);
  equal(b.finishDeadline(), now + 5000);
});
