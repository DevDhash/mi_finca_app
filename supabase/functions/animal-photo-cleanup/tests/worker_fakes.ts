...
import { claimRow, json, terminalRow } from "./adapter_fakes.ts";
...
export const environment: Record<string, string> = {
  ...
  SUPABASE_SECRET_KEYS: JSON.stringify({
    "animal-photo-cleanup": syntheticKey,
  }),
};
export const config = readConfig((name) => environment[name])!;
export const claimFor = (i: number) => ({
  ...claimRow,
  animal_id: uuid(100 + i),
  tombstone_sequence: String(i + 1),
});
export const evidenceFor = (i: number) => ({
  ...terminalRow,
  entity_id: uuid(100 + i),
  sequence: String(i + 1),
});
export function harness(
  replies: (Response | Fetch | Error)[],
...
export const request = (init: RequestInit = {}, query = "") =>
  new Request(`https://endpoint.invalid/animal-photo-cleanup${query}`, {
    method: "POST",
    headers: { apikey: syntheticKey },
    ...init,
  });
export const successfulJob = (
  i = 0,
) => [
  json([claimFor(i)]),
  json([evidenceFor(i)]),
