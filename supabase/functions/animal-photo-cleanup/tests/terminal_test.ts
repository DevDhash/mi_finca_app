...
test("timestamps retain microseconds and compare equivalent offsets", () => {
  equal(
    compareTerminal(job, {
      ...ledger,
      deletedAt: "2025-12-31T19:00:00.123456-05:00",
    }),
    null,
  );
  equal(
    timestampMicros("2026-01-01T00:00:00.123456Z")! -
      timestampMicros("2026-01-01T00:00:00.123455Z")!,
    1n,
  );
  equal(
    timestampMicros("2026-01-01 00:00:00.1+00:00"),
...
test("bigint is exact; Number and invalid job metadata are rejected", () => {
  ...
  equal(
    validJobMetadata({
      ...job,
      tombstoneSequence: Number(job.tombstoneSequence) as unknown as bigint,
    }),
    false,
  );
